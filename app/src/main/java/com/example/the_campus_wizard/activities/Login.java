...
public class Login extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        v_student_id = findViewById(R.id.loginID);
        v_password = findViewById(R.id.loginPassword);
        loginBackBtn = findViewById(R.id.loginBackBtn);
        forgotPass = findViewById(R.id.loginFrPass);

        //Deletes a previous user's data in order to prevent strange behaviour.
        ...
        succLoginBtn.setOnClickListener(v -> {
            String student_id = v_student_id.getText().toString().trim();
            String password = v_password.getText().toString().trim();

            if (!student_id.isEmpty() && !password.isEmpty()) {

                if (student_id.length() == 10 && !password.contains(" ") && student_id.matches("[0-9]+")) {
                    Intent StudentDashboard = Intents.dashboardIntent(this);

                    processLogin(student_id, password, StudentDashboard);
                } else {
                    if (student_id.length() != 10) {
                        Toast.makeText(this, "Your student id should be 10 characters long", Toast.LENGTH_LONG).show();
                    } else if (!student_id.matches("[0-9]+")) {
                        Toast.makeText(this, "The student id should be only numbers", Toast.LENGTH_LONG).show();
    ...
    private void processLogin(String student_id, String password, Intent StudentDashboard) {

        if (mAuth.getInstance().getCurrentUser() == null && AppSession.Session.userData.isEmpty()) {

            DocumentReference docRef = db.getInstance().collection("student").document(student_id);
                    ...
                    if (find_task.getResult().get("email") != null) {
                        ...
                        mAuth.getInstance().signInWithEmailAndPassword(email, password).addOnCompleteListener(login_task -> {
                            if (login_task.isSuccessful()) {
                                ...
                                db.getInstance().collection("student").document(student_id).get().addOnCompleteListener(task -> {
